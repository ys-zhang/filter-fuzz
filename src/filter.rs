...
mod utils {
    ...
    pub fn load_tf_model(model_name: &str) -> tf::SavedModelBundle {
    ...
    impl Model {
      ...
      pub fn new(name: &str, batch_size: usize, in_dim: usize, out_dim: usize) -> Self {
        let bundle = load_tf_model(name);
        Self {
          name: name.to_string(),
          in_dim,
          out_dim,
      ...
      pub fn predict(&self, xs: &[u8]) {
        let xs = tf::Tensor::from(xs);
      }
    }
...
mod tests {
    ...
    fn test_load_tf_bundle() {
        let bundle = load_tf_model("test-model");
