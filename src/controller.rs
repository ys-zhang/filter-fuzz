...
where
    ...
    E: Executor<I, R> + Send + 'static, // Executor
    I: Send + 'static,                  // Input
    R: Send + 'static,                  // Observed bitmap as Response
{
    ...
    pub fn start_flt_thread(&mut self, filter: F) -> Result<(), String> {
        if self.flt_thread.is_some() {
            Err("Filter thread still running".to_string())
        } else {
    ...
    pub fn start_exe_thread(&mut self, exe: E) -> Result<(), String> {
        if self.exe_thread.is_some() {
            Err("Execution thread still runnning".to_string())
        } else {
            let flt_out = self.chan_store.flt_out_rx.take().unwrap();
            let smpl_ch = self.chan_store.smpl_tx.take().unwrap();
            self.exe_thread = Some(thread::spawn(move || {
                let mut exe = exe;
                exe.run(flt_out, smpl_ch);
            }));
            Ok(())
        }
    }
